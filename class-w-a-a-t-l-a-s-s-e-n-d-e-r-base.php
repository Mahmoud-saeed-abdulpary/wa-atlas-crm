<?php
if(!class_exists("W_A_A_T_L_A_S_S_E_N_D_E_R_Base")) {
	class W_A_A_T_L_A_S_S_E_N_D_E_R_Base {
    	public $key = "9A3A5D9E99F0035D";
    	private $product_id = "2";
    	private $product_base = "wa-atlas";
    	private $server_host = "https://wpseoatlas.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_wa-atlas_fupc', function(){
					...
					if($this->is_theme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=wa-atlas_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..","wa-atlas").'</h1>');
